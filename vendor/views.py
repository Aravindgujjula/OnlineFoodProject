...
from pwn.models import CityModel , CuisineModel
...
def vendor_login_check(request):
    if request.method == "POST":
        ...
        return render(request, "vendor/login.html", {"error": "Vendor logged out"})


def vendor_register(request):
    return render(request,"vendor/vendor_register.html",{"cusine": CuisineModel.objects.all(),"city":CityModel.objects.all()})
...
def vendor_save(request):
    ...
    otp = 0000
    sta = "pending"
    VendorRegistrationModel(stall_name=sn, contact_1=c1, contact_2=c2, cuisine_type_id=cui, photo=ph, address=add,
                            vendor_city_id=cty, password=pas, OTP=otp, status=sta).save()
